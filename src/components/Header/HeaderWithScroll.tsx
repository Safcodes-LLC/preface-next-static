...
const HeaderWithScroll: FC<HeaderWithScrollProps> = ({ bottomBorder, className }) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const [megamenuData, postsData] = await Promise.all([
          getNavMegaMenu(),
          getAllPosts()
        ])
  ...
  return (
    <div 
      className={clsx(
        ...
        isScrolled 
          ? 'bg-white/95 backdrop-blur-sm border-b border-neutral-200 dark:bg-neutral-900/95 dark:border-neutral-700' 
          : 'bg-transparent',
        className
      )}
    >
      <div className="container">
        ...
        >
          ...
          <div className="ms-auto flex items-center justify-end gap-x-0.5">
            ...
            <div className={clsx('ms-6 me-3 hidden h-8 border-l lg:block', isScrolled ? 'border-neutral-200 dark:border-neutral-700' : 'border-white/20')}></div>
