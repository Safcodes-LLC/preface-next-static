...
const Card11: FC<Props> = ({ className, post, hiddenAuthor = false, ratio = 'aspect-5/3' }) => {
  ...
  return (
    <div
      className={clsx(
        'group post-card-11 relative flex flex-col justify-between rounded-3xl bg-white dark:bg-white/5',
        className
      )}
    ...
    >
      ...
      <div className="flex grow items-center gap-3 rounded-b-3xl border p-4">
        ...
        <div className="flex h-7 w-7 flex-shrink-0 cursor-pointer items-center justify-center rounded-full border border-[#E2E2E2] bg-white transition-transform duration-200 hover:translate-x-0.5 hover:bg-[#f3f3f3] dark:border-[#505050] dark:bg-[#0D0D0D] dark:hover:bg-[#1a1a1a]">
          <ArrowRightIcon className="h-3 w-3 text-[#919191] transition-colors duration-200 rtl:rotate-180 dark:text-[#707070] dark:hover:text-white" />
        </div>
      </div>
    </div>
