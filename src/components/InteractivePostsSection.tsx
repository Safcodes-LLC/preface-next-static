...
import { useLatestArticles, useLatestVideos, useTogglePostLike, useTogglePostBookmark, type Post } from '@/hooks/api/use-posts'
...
export default function InteractivePostsSection({ 
  ...
  const { data: latestPosts, isLoading: postsLoading, error: postsError, refetch: refetchPosts } = useLatestArticles(contentType === 'videos' ? 0 : limit)
  const { data: latestVideos, isLoading: videosLoading, error: videosError, refetch: refetchVideos } = useLatestVideos(contentType === 'posts' ? 0 : limit)
  
  const toggleLike = useTogglePostLike()
  const toggleBookmark = useTogglePostBookmark()

  // Determine which data to display based on content type
  const isLoading = contentType === 'mixed' ? (postsLoading || videosLoading) : 
                   contentType === 'videos' ? videosLoading : postsLoading
  const error = contentType === 'mixed' ? (postsError || videosError) : 
                contentType === 'videos' ? videosError : postsError
  
  const displayPosts = contentType === 'videos' ? (latestVideos?.data || videos) :
                       contentType === 'mixed' ? [...(latestPosts?.data || posts), ...(latestVideos?.data || videos)] :
      ...
      <div className="container">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {displayPosts.map((post) => (
            <article key={post.id} className="group bg-white dark:bg-neutral-900 rounded-2xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
              {/* Post Image */}
        ...
        <div className="text-center mt-8">
          ...
          >
            {isLoading ? 'Refreshing...' : `Refresh ${contentType === 'videos' ? 'Videos' : contentType === 'mixed' ? 'Content' : 'Posts'}`}
          </button>
        </div>
      </div>
    </div>
